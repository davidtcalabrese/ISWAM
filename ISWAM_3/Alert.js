 ...
 */
 class Alert {
  ...
  getAlertFields = alertProperties => {
    ...
    alertProps.push(util.formatDateTime(new Date(startTime)));
    alertProps.push(util.formatDateTime(new Date(endTime)));
  ...
  processAlert = async (zip, severityThreshold) => {
    ...
    if (alertData !== -1 && alert.isSevereEnough(severityThreshold)) {
      const data = alert.getAlertFields(alertData);
      return data;
    } else {
      return -1;
    }
